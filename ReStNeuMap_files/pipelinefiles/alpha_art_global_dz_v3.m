function alpha_art_global(Images,RealignmentFile,HeadMaskType, RepairType)
%====================================================================================
%====================================================================================
%
% Customized version of art_global v2.3 for ReissLab
%      z_thresh = 4;         which isn't used.
%      Percent_thresh = 1.3;  tighter to catch deep breaths
%      repair1_flag = 1;      Forces repair of scan 1.
%
% FORMAT art_global                         (v.2.3 for SPM2 and SPM5)
...
% batch script, e.g. as in the Repair and Compare button function.
...
% ----------------------------------------------------------------------
% v2.3, July 2008 PKM
%    Allows automatic adaptive threshold
%    Compatible SPM5 and SPM2
%    Lowered default thresholds, to catch more breath artifacts.
% v2.2, July 2007 PKM
...
%          end
    ...
    %if spm_ver == 'spm5'
        for i = 1:num_sess
            ...
            if realignfile == 1
                mvmt_file = spm_select(1,'any',['Select movement Goparams file for session' num2str(i) ':']);      
                M{i} = load(mvmt_file);
            end
        end
%     else   % spm2 version
...
%   % Rotation measure assumes voxel is 65 mm from origin of rotation.
    ...
    if RepairType == 2
        delsort = sort(delta);
        if delsort(round(0.7*nscans)) > mv_thresh
...
%LN fix on Aug, 18th: set visibility to 'off' to not show the plot.
figure('Units', 'normalized', 'Position', [0.2 0.2 0.6 0.7], 'visible', 'off');
...
if realignfile == 1
    ...
    legend('x mvmt', 'y mvmt', 'z mvmt','pitch','roll','yaw','Location','NorthEast');
	h = gca;
	set(h,'Ygrid','on');
...
legend('Fast motion','Location','best');
h = gca;
set(h,'Ygrid','on');
...
%set(gcf,'Units','normalized','PaperPosition',[0 0 10 7]);
set(gcf,'Units','normalized','PaperPosition',[0 0 22 18]);
    try
        saveas(gcf,figname);
    catch
