function estimatemodelMine(SubjectDir, Images, Names, Results, Deweight)
...
%      For batch version with multiple sessions:
%        In estimscript5, input a character array with each session name, 
%        where names are the same length.
...
%---------------------------------------------------
%nscans             = 186;                   % OR, will find from input. 
...
SPM.xX.K.HParam    = 120;                   % high-pass filter cutoff (secs) [Inf = no filtering]
...
SPM.xBF.T          = 28;            % number of time bins per scan
SPM.xBF.T0         = 14;            % reference time bin 
...
for j = 1:nsess
    ...
    %   [sess1,cond3]},
    %  {[sess2,cond1], etc.
    onset = {{[16 42 68 94 120 146],...
              [29 55 81 107 133 159],...
              },...
             {[16 42 68 94 120 146],...
              [29 55 81 107 133 159],...
              }};
    ...
    SPM.Sess(j).U(1).name = {'gogo'};                      % string in cell
    SPM.Sess(j).U(1).ons = onset{j}{1};      % onsets in scans or seconds - specified above
    SPM.Sess(j).U(1).dur = 12;                           % 0 for events, can be vector for epochs
    SPM.Sess(j).U(1).P.name = 'none';                   % 'none' | 'time' | 'other'    

    SPM.Sess(j).U(2).name = {'nogo'};                      % string in cell
    SPM.Sess(j).U(2).ons = onset{j}{2};      % onsets in scans or seconds - specified above
    SPM.Sess(j).U(2).dur = 12;                           % 0 for events, can be vector for epochs
...
%    When bigascript calls the ArtifactRepair program, it writes the
% files art_deweighted.txt and art_repaired.txt to the Images folder in
% each session study. For multiple sessions, each session will have an
% these files. If those files exist, the logic below
...
% T-contrasts
%---------------------------------------------------------------------------
% 1st column go, 2nd column nogo, 3rd column is the constant term in design matrix. 

con_vals{1}         = [ 1 -1   0];
con_name{1}         = 'go-go/nogo';

con_vals{2}         = [ -1 1   0];
con_name{2}         = 'go/nogo-go';

con_vals{3}         = [ 1  1   0];
con_name{3}         = 'active-rest';

  % Example contrasts for two session experiment
     % con_vals{1}         = [  1 -1     1 -1     0 0];
     % con_vals{2}         = [ -1  1    -1  1     0 0];
     % con_vals{3}         = [  1  1     1  1     0 0];
